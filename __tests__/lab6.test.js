describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for (i=0; i < prodItemsData.length; i++){
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    await page.reload();
    const buttons = await page.$$eval('product-item button', buttons => buttons.map(button => button.innerText));
    expect(buttons.every(text => text === "Remove from Cart")).toBeTruthy();
    const cartCount = await page.$eval('#cart-count', el => el.innerText);
    expect(cartCount).toBe("20");
  }, 10000);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    await page.reload();
    const buttons = await page.$$eval('product-item button', buttons => buttons.map(button => button.innerText));
    expect(buttons.every(text => text === "Add to Cart")).toBeTruthy();
    const cartCount = await page.$eval('#cart-count', el => el.innerText);
    expect(cartCount).toBe("0");
  }, 10000);
